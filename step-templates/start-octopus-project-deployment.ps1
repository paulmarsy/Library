...
class DeploymentFactory {
    ...
    [void] CreateRelease($releaseVersion) {
        if ([string]::IsNullOrWhiteSpace($releaseVersion)) {
            ...
            $releaseVersion = Invoke-OctopusApi "$($this.Project.Links.DeploymentProcess)/template?channel=$($this.Channel.Id)" | % NextVersionIncrement
        }
...
while ($tasksStillRunning) {
    # OD Web 1000ms poll loop; pollLoop.create($scope, function() { return reload() }, 1e3);
